...
public final class DataGenerator {
    ...
    public DataGenerator(Path tmp) throws IOException {
        final var process = new ProcessBuilder("java", "-DbundlerMainClass=net.minecraft.data.Main", "-jar", "../server.jar",
                "--reports", "--server")
    ...
    private void readStream(final InputStream stream, final Consumer<String> consumer) throws IOException {
        var length = 0;
        while (stream.available() > 0 && (length = stream.read(this.bytes, 0, this.bytes.length)) != -1) {
