...
public final class SlimeFormat {
    ...
    public static void load(final SculkServer server, final SculkWorld world, final WorldLoader.Slime loader) {
        try (final var inputStream = new DataInputStream(new ByteArrayInputStream(loader.bytes()))) {
            if (inputStream.readShort() != HEADER) throw new AssertionError();
    ...
    public static void save(final SculkServer server, final SculkWorld world, final OutputStream outputStream) {
        try (final var dataOutput = new DataOutputStream(outputStream)) {
            ...
            writeCompoundCompressed(dataOutput, CompoundBinaryTag.builder().put("tiles", tiles.build()).build());
            writeCompoundCompressed(dataOutput, CompoundBinaryTag.builder().put("entities", entities.build()).build());
    ...
    private static void writeChunks(final SculkServer server, final Collection<SculkChunk> chunks, final DataOutputStream dataOutput) throws IOException {
    ...
    private static void writeCompressed(final DataOutputStream outputStream, final byte[] bytes) throws IOException {
    ...
    private static void writeCompound(final DataOutputStream outputStream, final CompoundBinaryTag compound) throws IOException {
        try (final var byteStream = new ByteArrayOutputStream()) {
    ...
    private static void writeCompoundCompressed(final DataOutputStream outputStream, final CompoundBinaryTag compound) throws IOException {
        try (final var byteStream = new ByteArrayOutputStream();
