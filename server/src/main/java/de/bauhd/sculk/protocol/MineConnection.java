...
public final class MineConnection extends ChannelInboundHandlerAdapter implements Connection {
    ...
    public MineConnection(final SculkMinecraftServer server, final Channel channel) {
        this.server = server;
    ...
    public void play(GameProfile profile) {
        if (profile == null) {
            if (this.server.getConfig().mode() == MinecraftConfig.Mode.BUNGEECORD) {
                ...
                final var properties = (Property[]) SculkMinecraftServer.GSON.fromJson(arguments[3], TypeToken.getArray(Property.class).getType());
