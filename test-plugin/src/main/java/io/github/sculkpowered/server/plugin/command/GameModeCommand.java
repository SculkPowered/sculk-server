package io.github.sculkpowered.server.plugin.command;
...
public final class GameModeCommand {
  ...
  public static LiteralCommandNode<CommandSource> get() {
    return LiteralArgumentBuilder.<CommandSource>literal("gamemode")
        .then(RequiredArgumentBuilder.<CommandSource, Integer>argument("gamemode",
                IntegerArgumentType.integer(0, 3))
            .executes(context -> {
              if (context.getSource() instanceof Player player) {
                player.setGameMode(
                    GameMode.class.getEnumConstants()[IntegerArgumentType.getInteger(context,
                        "gamemode")]);
              }
              return 1;
            })
